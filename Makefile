...
test-race:
	@VERSION=$(VERSION) CGO_ENABLED=0 go test -mod=readonly -race -tags='ledger test_ledger_mock' ./...

test-cover:
	@CGO_ENABLED=0 go test -mod=readonly -timeout 30m -race -coverprofile=coverage.txt -covermode=atomic -tags='ledger test_ledger_mock' ./...
